...
export function GET_MEMORY_TOP(): usize {
    return __allocator_get_offset();
}

export function SET_MEMORY_TOP(offset: usize): void {
    __allocator_set_offset(GET_MEMORY_TOP(), offset);
}
...
function intersect(r: Ray, hit: Hit, locals: Locals): Hit {
    ...
    for (let i = 0; i < n; i++) {
        d = context.spheres.__unchecked_get(i).intersect(r, locals);
        if (d && d < t) {
            t = d;
            id = i;
        }
...
function radiance(r: Ray, depth: int, f: Vec, locals: Locals): Vec {
    ...
    var obj: Sphere = context.spheres.__unchecked_get(locals.hit.id); // the hit object
